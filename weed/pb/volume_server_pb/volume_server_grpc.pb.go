...
type VolumeServerClient interface {
	...
	UploadFile(ctx context.Context, opts ...grpc.CallOption) (VolumeServer_UploadFileClient, error)
...
func (c *volumeServerClient) UploadFile(ctx context.Context, opts ...grpc.CallOption) (VolumeServer_UploadFileClient, error) {
	stream, err := c.cc.NewStream(ctx, &VolumeServer_ServiceDesc.Streams[4], VolumeServer_UploadFile_FullMethodName, opts...)
	if err != nil {
		return nil, err
	}
	x := &volumeServerUploadFileClient{stream}
	return x, nil
}

type VolumeServer_UploadFileClient interface {
	Send(*UploadFileRequest) error
	CloseAndRecv() (*UploadFileResponse, error)
	grpc.ClientStream
}

type volumeServerUploadFileClient struct {
	grpc.ClientStream
}

func (x *volumeServerUploadFileClient) Send(m *UploadFileRequest) error {
	return x.ClientStream.SendMsg(m)
}

func (x *volumeServerUploadFileClient) CloseAndRecv() (*UploadFileResponse, error) {
	if err := x.ClientStream.CloseSend(); err != nil {
		return nil, err
	}
...
type VolumeServerServer interface {
	...
	UploadFile(VolumeServer_UploadFileServer) error
...
func (UnimplementedVolumeServerServer) UploadFile(VolumeServer_UploadFileServer) error {
...
func _VolumeServer_UploadFile_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(VolumeServerServer).UploadFile(&volumeServerUploadFileServer{stream})
}

type VolumeServer_UploadFileServer interface {
	SendAndClose(*UploadFileResponse) error
	Recv() (*UploadFileRequest, error)
	grpc.ServerStream
}

type volumeServerUploadFileServer struct {
	grpc.ServerStream
}

func (x *volumeServerUploadFileServer) SendAndClose(m *UploadFileResponse) error {
	return x.ServerStream.SendMsg(m)
}

func (x *volumeServerUploadFileServer) Recv() (*UploadFileRequest, error) {
...
var VolumeServer_ServiceDesc = grpc.ServiceDesc{
	...
	Streams: []grpc.StreamDesc{
		...
		},
		{
			...
			Handler:       _VolumeServer_UploadFile_Handler,
			ClientStreams: true,
		},
		{
